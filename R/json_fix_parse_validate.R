...
#' @export
    json_fix_parse_validate <- function(out, key_specs, na_values,
        ...
        # If the initial parse fails (e.g. due to a dangling comma),
        # strip trailing commas before } or ] and try again.
        if (is.null(parsed)) {
            repaired <- gsub(",\\s*([}\\]])", "\\1", rep_pair$txt)
            parsed <- tryCatch(
                jsonlite::fromJSON(repaired, simplifyVector = TRUE),
                error = function(e) NULL
            )
        }

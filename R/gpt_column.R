...
gpt_column <- function(
    ...
    for (i in seq_len(n)) {
        ...
        if (!isTRUE(rp$ok)) {
            parsed_results[[i]] <- if (relaxed && is.null(expected_keys)) out else
                if (!is.null(expected_keys)) setNames(rep(NA, length(expected_keys)), expected_keys) else out
            invalid_flags[i]    <- TRUE
        ...
        if (!is.null(expected_keys) && !setequal(names(x), expected_keys)) {
            ...
            if (!isTRUE(keep_unexpected_keys)) {
                ...
                if (length(dropped)) {
                    ...
                    meta_df <- if (is.null(meta_df)) meta_drop else tryCatch(
                ...
                if (length(extra_names)) {
                    ...
                    meta_df <- if (is.null(meta_df)) meta_keep else tryCatch(
        ...
        if (isTRUE(keep_unexpected_keys)) {
            parsed_json <- vapply(
                ...
                function(x) {
                    ...
                    if (is.character(x) && length(x) == 1L) return(x)  # relaxed scalar passthrough
